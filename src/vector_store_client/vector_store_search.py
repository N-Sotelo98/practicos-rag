...
"""
Módulo para manejar la búsqueda de embeddings en Qdrant adaptado a vectores estándar y OpenAI (vectores de 384 y 1536 dimensiones).
"""
...
def search_embeddings(client, index_name, query_embedding, limit=5):
    ...
    Args:
        ...
        query_embedding: Embedding de la consulta (vector plano de 384 dimensiones).
...
def search_embeddings_openai(client, index_name, query_embedding, limit=5):
    """
    Realiza una búsqueda en Qdrant usando un vector generado por OpenAI (1536 dimensiones).
    ...
    try:
        ...
        if not isinstance(query_embedding, list) or len(query_embedding) != 1536:
            raise ValueError(
                f"El vector de consulta debe ser una lista de 1536 dimensiones. "
                f"Recibido: {type(query_embedding)} con tamaño {len(query_embedding)}"
            )
        ...
        results = client.query_points(
            ...
            using="default",  # Nombre explícito del vector
            with_payload=True,   # Incluir los payloads
            with_vectors=False    # Opcional: no solicitar vectores
        )
...
def format_search_results_openai(query, results):
    """
    Formatea y muestra los resultados de una búsqueda de embeddings generados por OpenAI.
