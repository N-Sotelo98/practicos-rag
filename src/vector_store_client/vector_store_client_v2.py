...
def insert_embeddings_openai(client, index_name, embeddings, metadata_list, batch_size=100):
    """
    Inserta embeddings generados por OpenAI y metadatos en Qdrant con validaciones específicas.

    Args:
        client: Cliente de Qdrant.
        index_name: Nombre de la colección.
        embeddings: Lista de embeddings generados por OpenAI.
        metadata_list: Lista de metadatos asociados a cada embedding.
        batch_size: Tamaño del lote para inserción.
    """
    valid_points = []
    ...
    for i, (embedding, metadata) in enumerate(zip(embeddings, metadata_list)):
        try:
            ...
            if not isinstance(embedding, list) or len(embedding) != 1536:
                raise ValueError(
                    f"Embedding inválido en índice {i}: tamaño {len(embedding)} esperado 1536.")
